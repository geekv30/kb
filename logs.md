...
|---|---|---|---|
| 2026-05-19 | Modal centering-drift fix | #123 | User reported `(-3, 3) → (0, 0)` drift on /edit pages when opening modals (NewCategoryModal via FileExplorerNav 3-dot). Root cause: `Dialog.Content` combined translate-based centering with the scale keyframe on the same transform stack; Radix's scroll-lock-on-open shifted viewport math for `left-1/2`/`top-1/2` mid-paint. Fix: centering delegated to a non-animated `grid place-items-center` wrapper; Content animates only `scale(0.96)→1` + opacity around `origin-center`. Inline mode + reduced-motion paths untouched. |
