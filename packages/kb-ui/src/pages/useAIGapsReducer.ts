...
// `Patterns/KB AI Gaps → Interactive` story.
//
// The reducer is intentionally exported **separately** from the hook so it
// can be unit-tested or re-composed by another story without pulling in
// React. It is a pure function: no side effects, no scroll calls. All DOM
// effects (scrolling into view, focus, keyboard wiring) live in the story
// component's `useEffect`s.
//
// Not exported from `src/index.ts` — story-internal, not public API.
